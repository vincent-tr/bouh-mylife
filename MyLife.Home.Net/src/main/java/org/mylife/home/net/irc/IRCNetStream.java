...
public class IRCNetStream {
	 ...
	 * @param realname
	 */
	public IRCNetStream(String pass, String nick, String username,
			String realname) {
	 ...
	 */
	public void prepare(OutputStream s) throws IOException {
		level = 1;
		out = new PrintWriter(new OutputStreamWriter(s, encoding));
		register();
	}
	...
	private void register() {
		if (pass != null)
			send("PASS " + pass);
		send("NICK " + nick);
		send("USER " + username + " " + username + " " + username + " :"
				+ realname);
	}
	 ...
	 */
	public synchronized void get(String line) {
		IRCParser p;
		try {
	...
	public String toString() {
		return getClass().getName() + "[" + username + "," + nick + ","
