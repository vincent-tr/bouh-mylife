...
public class SelectorListener extends Listener {
	...
	private static final Logger logger = Logger
			.getLogger(SelectorListener.class.getName());
	...
	public SelectorListener(IrcServerMBean jircd, String address, int port,
	...
	public boolean bind() {
		try {
			...
			serverChannel.socket().bind(
					new InetSocketAddress(boundAddress, boundPort), BACKLOG);
	...
	protected void waitForActivity() throws IOException {
		synchronized (selector) {
			...
			for (Iterator<SelectionKey> iter = selector.selectedKeys()
					.iterator(); iter.hasNext(); iter.remove()) {
	...
	private void doAccept() throws IOException {
		...
		if (clientChannel != null) {
			...
			Connection.Handler handler = new Connection.Handler(jircd,
					connection);
	...
	private void doSocketWrite(SelectionKey key) throws IOException {
		...
		if (client.responseQueue.isEmpty()
				&& !client.currentResponse.hasRemaining()
				&& !buffer.hasRemaining()) {
	...
	public void close() {
		super.close();
		synchronized (selector) {
			for (Iterator<SelectionKey> iter = selector.keys().iterator(); iter
					.hasNext();) {
				SelectionKey key = iter.next();
				if (key == serverKey) {
					try {
						...
						logger.log(Level.WARNING,
								"Server channel close exception", ioe);
					}
				} else {
					disconnect(key);
				}
			...
			try {
				...
				logger.log(Level.WARNING, "Selector close exception", ioe);
			}
		}
	}
	...
	class KeyConnection extends Connection {
		...
		private final CharBuffer currentRequest = CharBuffer
				.allocate(Constants.MAX_MESSAGE_SIZE);
		private final CharsetEncoder encoder = CHARSET.newEncoder();
		private final Queue<String> responseQueue = new ConcurrentLinkedQueue<String>();
		private final CharBuffer currentResponse = CharBuffer
				.allocate(Constants.MAX_MESSAGE_SIZE);
		...
		private void readRequestFromReceiveBuffer() {
			...
			/* CoderResult rc = */decoder.decode(inBuffer, currentRequest,
					false);
			currentRequest.flip();
			...
			while (currentRequest.hasRemaining()) {
				...
				if (ch == '\r' || ch == '\n') {
					...
					String line = currentRequest.subSequence(0, pos - 1)
							.toString();
					if (ch == '\r' && hasRemaining
							&& currentRequest.get(pos) == '\n')
					...
					try {
						...
						if (logger.isLoggable(Level.FINE))
							logger.fine("Message received from " + toString()
									+ "\n\t" + line);
						if (line.length() > 0)
							handler.handleLine(line);
					} catch (RuntimeException e) {
						logger.log(Level.WARNING, "Exception occured in "
								+ toString(), e);
		...
		private void processOutQueue() {
			...
			for (String str; !currentResponse.hasRemaining()
					&& (str = (String) responseQueue.poll()) != null;) {
				...
				linesSent++;
				if (logger.isLoggable(Level.FINE))
					logger.fine("Message sent to " + toString() + "\n\t" + str);
			}
			outBuffer.flip();
		}

		private void writeResponseToSendBuffer() {
			/* CoderResult rc = */encoder.encode(currentResponse, outBuffer,
					false);
		}
		...
		protected void close() {
			logger.fine("Closing connection " + toString());
			try {
				key.channel().close();
			} catch (IOException e) {
				logger.log(Level.FINE, "Exception on channel close", e);
			} finally {
