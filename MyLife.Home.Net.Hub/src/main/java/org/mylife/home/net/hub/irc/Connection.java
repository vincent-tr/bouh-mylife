...
public abstract class Connection {
	...
	public String toString() {
		return '[' + socket.getInetAddress().getHostAddress() + ':'
				+ socket.getPort() + ',' + socket.getClass().getName() + ','
				+ handler + ']';
	}

	/**
	 * Abstraction for message-layer protocol. This class is responsible for
	 * maintaining a connection, and dispatching Messages. The Entity of a
	 * Handler is UnregisteredEntity until it logins.
	 */
	public static class Handler {
		protected final Connection connection;
		protected final IrcServerMBean jircd; // the server we are connected to
		...
		public Handler(IrcServerMBean jircd, Connection connection) {
			if (connection == null)
		...
		/**
		 * Registers/logs-in using the specified entity. This should be used by
		 * to register Entity implementations.
		 */
		public final void login(ConnectedEntity newEntity) {
			...
			if (entity instanceof UnregisteredEntity) {
				if (newEntity.getHandler() != this)
					throw new IllegalArgumentException("The handler of "
							+ newEntity.toString() + " must be " + toString()
							+ " but it was "
							+ newEntity.getHandler().toString());
				if (newEntity.getServer() != entity.getServer())
					throw new IllegalArgumentException("The server of "
							+ newEntity.toString() + " must be "
							+ entity.getServer().toString() + " but it was "
							+ newEntity.getServer().toString());
				entity = newEntity;
		...
		public final void setMessageFactory(MessageFactory messageFactory) {
			if (messageFactory == null)
				throw new NullPointerException(
						"The message factory cannot be null");
		...
		/**
		 * Pings this connection. Returns false on ping timeout.
		 */
		public final boolean ping() {
			final long currentTime = System.currentTimeMillis();
			if (lastPong >= lastPing) { // got a response previously
				if (currentTime - lastPong > pingTimeout) { // more than timeout
															// seconds?
					// send ping
		...
		final void handleLine(String line) {
			// if there has been a pong since the last ping, then don't try to
			// ping again
			...
			if (line.length() > Constants.MAX_MESSAGE_LENGTH)
				line = line.substring(0, Constants.MAX_MESSAGE_LENGTH); // max
																		// length
																		// per
																		// RFC
