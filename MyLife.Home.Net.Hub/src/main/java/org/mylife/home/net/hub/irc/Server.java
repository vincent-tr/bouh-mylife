...
public class Server extends Source {
	...
	protected final Connection connection; // the connection used if it's linked to me
	 ...
	 */
	public Server(String name, int token, String description, Server route, Connection connection, Object client) {
		if(route == null)
			throw new NullPointerException("The route cannot be null");
		if(route.getNetwork() == null)
			throw new NullPointerException("The route cannot have a null network");
		this.name = name;
		this.token = token;
	...
	/**
	 * Constructs an IRC server on an IRC network.
	 * This should only be used to construct the local server.
	 */
