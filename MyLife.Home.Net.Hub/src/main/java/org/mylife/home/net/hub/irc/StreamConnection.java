...
public class StreamConnection extends Connection implements Runnable {
	...
	public StreamConnection(Socket socket, ConnectionManager manager,
		...
		input = new LineNumberReader(new InputStreamReader(inputStats,
				Constants.CHARSET), Constants.MAX_MESSAGE_SIZE);
		output = new BufferedWriter(new OutputStreamWriter(outputStats,
				Constants.CHARSET), Constants.MAX_MESSAGE_SIZE);
		threadPool = pool;
		logger.fine("Initiated connection " + toString());
	...
	public final void run() {
		while (!Thread.interrupted()) {
			try {
				...
				logger.log(Level.WARNING, "Exception occured in " + toString(),
						e);
				handler.getEntity().disconnect(e.getMessage());
				return;
			}
	...
	public void writeLine(String text) {
		...
		try {
			...
			logger.log(Level.FINE, "Exception occurred while sending message",
					e);
		}
	}
	...
	protected void close() {
		...
		try {
			...
			logger.log(Level.FINE, "Exception on socket close", e);
		} finally {
